...
${manualAgenda.trim() ? `\nMEETING AGENDA/CONTEXT (use this to help guide your summary): ${manualAgenda}` : ''}`
  ...
  const recordPanelContent = (
    <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm flex flex-col transition-colors duration-300">
      {/* Live / Upload sub-tabs */}
      <div className="flex border-b border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50">
        <button
          onClick={() => !isProcessing && setActiveTab('live')}
          className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'live' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'} ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'upload' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'} ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}
          disabled={isProcessing}
        >
      ...
      <div className="p-4 md:p-5 flex flex-col items-center justify-center min-h-[250px] relative">
        {isProcessing && activeTab === 'live' ? (
          <>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 text-center">Processing transcript against LLaMA...</p>
          </>
        ) : activeTab === 'live' ? (
          <>
            <div className="relative flex items-center justify-center">
              {isRecording && (
                <button
                  ...
                  className="absolute -left-16 sm:-left-20 w-10 h-10 rounded-full bg-white dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white flex items-center justify-center transition-all shadow-md border border-gray-200 dark:border-zinc-700 animate-in fade-in zoom-in"
          ...
          <>
             <div
              className={`w-full rounded-2xl border-2 border-dashed p-6 flex flex-col items-center justify-center text-center transition-colors shadow-xs ${isProcessing ? 'border-indigo-500/30 bg-indigo-500/5' : 'border-gray-200 dark:border-zinc-700 hover:border-indigo-500/50 hover:bg-gray-50 dark:hover:bg-zinc-900'}`}
            ...
            >
              {isProcessing ? (
                ...
                <FileAudio className="w-10 h-10 text-zinc-400 dark:text-zinc-500 mb-3" />
              )}
              ...
              {!isProcessing && (
                <button
                  ...
                  className="mt-4 px-4 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg text-xs font-medium text-zinc-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-700 hover:text-zinc-900 dark:hover:text-white transition-colors shadow-sm"
                >
                  Browse Files
                </button>
              )}
      ...
      {!isProcessing && (
        <div className="px-6 pb-6 pt-2 bg-white dark:bg-zinc-900 flex flex-col gap-4 border-t border-gray-100 dark:border-zinc-800/50 mt-2 transition-colors duration-300">
          <div>
            <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5 block transition-colors duration-300">
            ...
            <input
              ...
              className="w-full bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-900 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500/50 transition-all shadow-xs"
            />
          </div>
          <div>
            <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1.5 block transition-colors duration-300">
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-900 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500/50 transition-all resize-none shadow-xs"
            />
          </div>
        </div>
      )}
  ...
  const historyListContent = (
    <div className="flex-1 overflow-y-auto min-h-0 bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm flex flex-col transition-colors duration-300">
      <div className="px-5 py-4 border-b border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50 shrink-0">
        <h3 className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider transition-colors">Recent Meetings History</h3>
      </div>
            ...
            {recentMeetings.map((doc) => (
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-white dark:bg-zinc-800/80 flex items-center justify-center shrink-0 group-hover:bg-indigo-500/20 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors border border-gray-200 dark:border-zinc-700/50 shadow-xs">
                    <Mic className="w-4 h-4 text-zinc-500 dark:text-zinc-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1.5 text-[11px] font-medium text-zinc-500 dark:text-zinc-500">
                ...
                {doc.summary && (
                  <p className="text-xs text-zinc-600 dark:text-zinc-400 line-clamp-2 leading-relaxed mt-auto transition-colors">
                    {doc.summary}
                  </p>
                )}
                <div className="mt-3 flex items-center justify-between pt-3 border-t border-gray-100 dark:border-zinc-800/50 transition-colors">
  ...
  const summaryPanelContent = (
    activeMeeting ? (
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 flex flex-col h-full overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-sm transition-colors duration-300">

        {/* Header */}
        <div className="px-5 py-4 md:p-6 border-b border-gray-100 dark:border-zinc-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50 dark:bg-zinc-900/50 transition-colors duration-300">
              ...
              {isEditingTitle ? (
                <div className="flex items-center gap-2 max-w-sm">
                  <input
                    ...
                    autoFocus
                    className="bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-700 rounded-lg px-3 py-1 flex-1 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-indigo-500 transition-all shadow-xs"
                  />
                  <button
                ...
                <div className="flex items-center gap-2 group cursor-pointer max-w-[200px] sm:max-w-xs md:max-w-sm" onClick={() => {
                  ...
                  <Pencil className="w-3.5 h-3.5 text-zinc-400 dark:text-zinc-600 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors shrink-0" />
                </div>
              )}
              <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mt-1.5 flex-wrap transition-colors duration-300">
        ...
        <div className="p-4 md:p-5 flex-1 overflow-y-auto bg-gray-50/30 dark:bg-zinc-950/20 transition-colors duration-300">
          <div className="max-w-none space-y-8">

            {/* Summary */}
            <section>
              <h3 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mb-3 flex items-center gap-2 transition-colors duration-300">
                <BookOpen className="w-4 h-4 text-indigo-500 dark:text-indigo-400" />
                Executive Summary
              </h3>
              <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800/80 p-5 rounded-xl shadow-xs transition-colors duration-300">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {activeMeeting.actionItems?.length > 0 ? (
                  activeMeeting.actionItems.map((item, idx) => (
                    <li key={idx} className="flex gap-3 p-4 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800/80 rounded-xl hover:border-gray-300 dark:hover:border-zinc-700 transition-all shadow-xs">
                      <div className="w-5 h-5 rounded border border-gray-300 dark:border-zinc-600 flex items-center justify-center shrink-0 mt-0.5 bg-gray-50 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500">
                      </div>
                      <span className="text-zinc-700 dark:text-zinc-300 text-sm leading-relaxed transition-colors">{item}</span>
                    </li>
                  ))
                ) : (
                  <li className="p-4 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800/80 rounded-xl text-zinc-500 text-sm col-span-2 shadow-xs transition-colors duration-300">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeMeeting.attendees?.length > 0 ? (
                    activeMeeting.attendees.map((att, idx) => (
                      <span key={idx} className="bg-gray-100 dark:bg-zinc-800/80 border border-gray-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 px-3 py-1 rounded-full text-xs font-semibold shadow-xs transition-all">
                        {att}
                      </span>
                    ))
                  ) : (
        ...
        {/* Action Buttons Row */}
        <div className="p-4 border-t border-gray-100 dark:border-zinc-800 flex flex-wrap gap-3 bg-white dark:bg-zinc-900 transition-colors duration-300">
          {liveAudioBlobUrl && activeMeeting.inputType === 'live' && (
            <a
              href={liveAudioBlobUrl}
              download={`meeting-${activeMeeting.id}.webm`}
              className="inline-flex items-center gap-2 px-3 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 text-xs font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-700 hover:text-zinc-900 dark:hover:text-white transition-all shadow-xs"
            >
        ...
        {/* Ask AI Section */}
        <div className="border-t border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950/50 transition-colors duration-300">
          <div className="px-5 py-3 border-b border-gray-100 dark:border-zinc-800/60 flex items-center gap-2 transition-colors duration-300">
            ...
            <div className="px-5 py-4 max-h-60 overflow-y-auto space-y-4">
              {askResponses.map((msg, i) => (
                <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[14px] leading-relaxed transition-all duration-300 ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-sm shadow-sm' : 'bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700/50 text-zinc-800 dark:text-zinc-200 rounded-tl-sm shadow-sm'}`}>
                    {msg.role === 'user' ? (
                      msg.content
                    ) : (
                      <ReactMarkdown
              ...
              {isAsking && (
                <div className="flex flex-col items-start">
                  ...
                  <div className="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700/50 px-4 py-3 rounded-2xl rounded-tl-sm shadow-sm flex items-center gap-3 transition-all duration-300">
                    <Loader2 className="w-4 h-4 text-indigo-600 dark:text-indigo-400 animate-spin" />
                    <span className="text-sm text-zinc-500 dark:text-zinc-400">Analyzing...</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <form onSubmit={handleAskAI} className="p-4 flex gap-3 bg-white dark:bg-zinc-900 border-t border-gray-100 dark:border-zinc-800/60 transition-colors duration-300">
            <div className="relative flex-1">
              <input
                ...
                className="w-full pl-4 pr-10 py-2 bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 text-sm rounded-xl focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all disabled:opacity-50 placeholder-zinc-400 dark:placeholder-zinc-500 shadow-xs"
              />
              <CornerDownRight className="w-4 h-4 text-zinc-400 dark:text-zinc-600 absolute right-4 top-1/2 -translate-y-1/2 transition-colors" />
            </div>
            <button
              type="submit"
  ...
  // --------------------------------------------------------------------------
  return (
    <div className="flex flex-col min-h-full bg-gray-50 dark:bg-zinc-950 pb-6 md:pb-10 text-zinc-900 dark:text-zinc-100 font-sans transition-colors duration-300">
      ...
      {headerNode && createPortal(
        <div className="flex items-center justify-between w-full h-full animate-in fade-in transition-all duration-300">
          <div className="flex flex-col justify-center shrink-0">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-[10px] leading-none hidden md:block mt-1 transition-colors duration-300">Transcribe & analyze meetings instantly with Whisper & LLaMA AI.</p>
          </div>
      ...
      <div className="md:hidden px-4 pt-4 pb-2 transition-colors duration-300">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-[11px] mt-1.5 transition-colors duration-300">Transcribe & analyze meetings instantly with Whisper & LLaMA AI.</p>
      </div>

      {/* ── Mobile 3-tab bar — only visible below lg breakpoint */}
      <div className="lg:hidden mt-3 mx-4 flex rounded-xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-1 gap-1 shadow-xs transition-colors duration-300">
          {[
