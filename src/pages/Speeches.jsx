...
For official letters, use proper formal structure.`;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--bg-app)] text-[var(--text-main)] font-sans transition-colors duration-300 overflow-hidden">
      ...
      {headerNode && createPortal(
        <div className="flex items-center justify-between w-full h-full animate-in fade-in">
          <div className="flex flex-col justify-center shrink-0">
            <h1 className="text-base font-bold text-[var(--text-main)] tracking-tight leading-none">AI Speechwriter</h1>
            <p className="text-[var(--text-muted)] text-[10px] leading-none hidden md:block mt-1">Draft speeches, letters, and statements.</p>
          </div>
        </div>,
        headerNode
      )}
      ...
      <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-[var(--border-main)] bg-[var(--bg-app)]/80 backdrop-blur-md sticky top-0 z-30 shrink-0">
        <div className="flex items-center gap-2">
          <Pencil className="w-4 h-4 text-[var(--accent-primary)]" />
          <span className="text-sm font-semibold text-[var(--text-main)]">AI Speechwriter</span>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)}
            className={`p-2 rounded-lg border transition-all ${mobileSidebarOpen ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-[var(--bg-subtle)] border-[var(--border-main)] text-[var(--text-muted)]'}`}
          >
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden relative">
        ...
        <aside className={`
          absolute inset-0 z-20 lg:relative lg:flex lg:flex-col lg:w-80 border-r border-[var(--border-main)] bg-[var(--bg-app)] transition-transform duration-300 transform
          ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}>
          <div className="flex flex-col h-full">
            <div className="p-4 border-b border-[var(--border-main)] bg-[var(--bg-app)] sticky top-0 z-10 hidden lg:block">
              <button
                onClick={handleNewDraft}
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4 space-y-4">
              <div className="px-2 flex items-center justify-between mb-2">
                <h3 className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em]">Saved Drafts</h3>
                <span className="bg-[var(--bg-subtle)] text-[var(--text-dim)] text-[10px] px-1.5 py-0.5 rounded-full font-bold">{speeches.length}</span>
              </div>
                  ...
                  {speeches.map(speech => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-xl border transition-all relative group ${activeSpeech?.id === speech.id
                          ...
                          : 'bg-[var(--bg-subtle)]/40 border-[var(--border-main)]/60 hover:border-indigo-500/30'
                        }`}
                    >
                      <div className="flex items-center gap-3 mb-2">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${activeSpeech?.id === speech.id ? 'bg-indigo-600 text-white shadow-lg' : 'bg-[var(--bg-subtle)] text-[var(--text-dim)]'
                          }`}>
                          {getTypeIcon(speech.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-bold truncate leading-tight ${activeSpeech?.id === speech.id ? 'text-white' : 'text-[var(--text-main)]'}`}>
                            {speech.title || 'Untitled'}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] font-bold text-[var(--text-muted)] bg-[var(--bg-subtle)] px-1.5 py-0.5 rounded border border-[var(--border-main)]/80">
                          {speech.type?.toUpperCase()}
                        </span>
                        <span className="text-[10px] text-[var(--text-dim)] font-medium">{formatDate(speech.createdAt)}</span>
                      </div>
                    </button>
                  ))}
                </div>
        ...
        {mobileSidebarOpen && (
          <div
            className="absolute inset-0 bg-[var(--bg-app)]"
            onClick={() => setMobileSidebarOpen(false)}
          />
        )}

        {/* MAIN CONTENT AREA */}
        <main className="flex-1 flex flex-col min-w-0 bg-[var(--bg-app)] overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 scrollbar-hide animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="max-w-2xl mx-auto pb-10">
                <header className="mb-8">
                  <h2 className="text-2xl font-black text-[var(--text-main)] tracking-tight">Create New Draft</h2>
                  <p className="text-[var(--text-muted)] text-sm mt-1">Set your preferences for the perfect draft.</p>
                </header>

                <div className="space-y-8">
                  {/* Draft Type */}
                  <section>
                    <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4">Draft Type</label>
                      ...
                      {DRAFT_TYPES.map(type => (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center gap-2.5 p-4 rounded-2xl border transition-all duration-300 ${draftType === type.id
                              ...
                              : 'bg-[var(--bg-subtle)]/50 border-[var(--border-main)] text-[var(--text-muted)] hover:border-zinc-700 hover:text-[var(--text-main)]'
                            }`}
                        >
                          <type.icon className={`w-5 h-5 ${draftType === type.id ? 'text-white' : 'text-[var(--text-muted)]'}`} />
                  ...
                  {/* Topic */}
                  <section>
                    <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4">Topic / Occasion</label>
                    <input
                      type="text"
                      value={topic}
                      onChange={e => setTopic(e.target.value)}
                      placeholder="e.g. Inauguration of NH-48 Highway..."
                      className="w-full bg-[var(--bg-subtle)] border-2 border-[var(--border-main)] text-[var(--text-main)] rounded-2xl px-5 py-4 focus:border-indigo-500 outline-none transition-all placeholder:text-[var(--text-dim)] text-base"
                    />
                  </section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* Tone */}
                    <section>
                      <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4">Tone</label>
                        ...
                        {TONES.map(t => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${tone === t
                                ...
                                : 'bg-[var(--bg-subtle)] border-[var(--border-main)] text-[var(--text-muted)] hover:border-zinc-700'
                              }`}
                          >
                            {t}
                          </button>
                    ...
                    {/* Length */}
                    <section>
                      <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4">Length</label>
                      <div className="flex flex-wrap gap-2">
                        {LENGTHS.map(l => (
                          <button
                            key={l.id}
                  ...
                  {/* Context */}
                  <section>
                    <label className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4">Additional Context</label>
                    <textarea
                      value={context}
                      onChange={e => setContext(e.target.value)}
                      placeholder="Notes, statistics, or specific talking points..."
                      rows={4}
                      className="w-full bg-[var(--bg-subtle)] border-2 border-[var(--border-main)] text-[var(--text-main)] rounded-2xl px-5 py-4 focus:border-indigo-500 outline-none transition-all placeholder:text-[var(--text-dim)] resize-none text-base"
                    />
                  </section>
                </div>
          ...
          {step === 2 && (
            <div className="flex-1 flex flex-col items-center justify-center p-8 animate-in zoom-in-95 duration-500">
              ...
              <h3 className="text-2xl font-black text-[var(--text-main)] mt-8 mb-2 tracking-tight">Drafting In Progress</h3>
              <p className="text-[var(--text-muted)] text-center max-w-70">Our legislative engine is crafting your {draftType.toLowerCase()} response...</p>
          ...
          {step === 3 && activeSpeech && (
            <div className="flex-1 flex flex-col min-h-0 bg-[var(--bg-app)] animate-in slide-in-from-right-4 duration-500">

              {/* Toolbar */}
              <div className="px-4 py-4 md:px-6 md:py-5 border-b border-[var(--border-main)] flex flex-wrap items-center justify-between gap-4 sticky top-0 z-10 bg-[var(--bg-app)]/80 backdrop-blur-md shrink-0">
                <div className="flex-1 min-w-0 pr-2">
                  {isEditingTitle ? (
                    <input
                      ...
                      className="w-full bg-[var(--bg-subtle)] border-2 border-indigo-500/50 text-[var(--text-main)] text-base md:text-lg font-black rounded-xl px-4 py-2 focus:outline-none"
                    />
                  ) : (
                    <div className="flex flex-col">
                      ...
                      <h2
                        className="text-lg md:text-xl font-black text-[var(--text-main)] truncate cursor-pointer hover:text-indigo-400 transition-colors flex items-center gap-2"
                        onClick={() => { setEditTitle(activeSpeech.title); setIsEditingTitle(true); }}
                      >
                        {activeSpeech.title}
                        < Pencil className="w-3.5 h-3.5 text-[var(--text-dim)] shrink-0" />
                      </h2>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 m-auto md:m-0">
                  <button onClick={handleCopyToClipboard} className="p-2.5 bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-muted)] rounded-xl hover:text-white hover:border-zinc-700 transition-all active:scale-90" title="Copy Content">
                    <Copy className="w-4.5 h-4.5" />
                  </button>
                  <button onClick={handleDownload} className="p-2.5 bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-muted)] rounded-xl hover:text-[var(--text-main)] hover:border-indigo-500/30 transition-all active:scale-90" title="Download Text">
              ...
              {/* Editor Container */}
              <div className="flex-1 min-h-0 bg-[var(--bg-app)] overflow-hidden relative group">
                <style>{`
                  ...
                  .speeches-quill-container .ql-stroke { stroke: var(--text-muted) !important; }
                  .speeches-quill-container .ql-fill { fill: var(--text-muted) !important; }
                  .speeches-quill-container .ql-picker { color: var(--text-muted) !important; font-weight: bold !important; }
                  .speeches-quill-container .ql-active .ql-stroke, 
                  .speeches-quill-container .ql-active .ql-fill,
                  .speeches-quill-container button:hover .ql-stroke { stroke: var(--accent-primary) !important; }
